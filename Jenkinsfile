pipeline {
    ...
    environment {
        DOCKER_IMAGE = 'vetApp'
        EC2_PUBLIC_IP = '3.145.140.99'
        REPO_URL = 'https://github.com/gq80132/P4P.git'
    }
        ...
        stage('Deploy Container') {
            steps {
                ...
                sh '''
                    ...
                    docker stop vetApp || true
                    docker rm vetApp || true

                    # Run new container
                    docker run -d --name vetApp -p 9090:9090 ${DOCKER_IMAGE}:latest
    ...
    post {
        ...
        failure {
            ...
            sh 'docker logs vetApp|| echo "Container not running"'
        }
    }
}